...
namespace PKK
{
    public partial class Hasil_Suara : Form
    {
        public Hasil_Suara()
        {
            InitializeComponent();
        ...
        Color.FromArgb(155, 89, 182)
            ...
            // Label total suara
            lblTotalSuara.Text = $"Total Suara Masuk: {totalSuara} / {totalPemilih}";
            lblTotalSuara.Font = new Font("Segoe UI", 11, FontStyle.Bold);
            lblTotalSuara.ForeColor = Color.White;

            // Label pemenang
            int suaraTerbanyak = Convert.ToInt32(dt.Rows[0]["Jumlah_Suara"]);
            double persen = (double)suaraTerbanyak / totalSuara * 100;
            lblPemenang.Text = $"🏆 Pemenang: \n{dt.Rows[0]["nama_ketua"]} & {dt.Rows[0]["nama_wakil"]} ({suaraTerbanyak} suara / {persen:F1}%)";
            lblPemenang.Font = new Font("Segoe UI", 12, FontStyle.Bold);
            lblPemenang.ForeColor = Color.Goldenrod;
        ...
        private void Hasil_Suara_Load(object sender, EventArgs e)
        {
            TampilkanPieChart();
            tampildata();
        }
